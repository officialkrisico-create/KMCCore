...
/** /kmctournament <start|stop|status|reset> */
public class TournamentCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!sender.hasPermission("kmc.tournament.admin")) { sender.sendMessage(MessageUtil.get("no-permission")); return true; }
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "reset" -> {
                ...
                sender.sendMessage(MessageUtil.get("tournament.reset"));
            }
            case "status" -> {
                var tm = plugin.getTournamentManager();
                sender.sendMessage(MessageUtil.get("tournament.status-header"));
                sender.sendMessage(tm.isActive() ? MessageUtil.get("tournament.status-active") : MessageUtil.get("tournament.status-inactive"));
    ...
    private void usage(CommandSender s) { s.sendMessage(MessageUtil.get("invalid-usage").replace("{usage}", "/kmctournament <start|stop|status|reset>")); }
}
